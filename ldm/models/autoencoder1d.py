...
class AutoencoderKL(pl.LightningModule):
    ...
    def get_input(self, batch, k):
        ...
        return [x, f0, pitches, notedurs, notetypes, ph,ph_durs]

    def training_step(self, batch, batch_idx, optimizer_idx):
        inputs = self.get_input(batch, self.image_key)
        x, f0, pitches, notedurs, notetypes, ph, ph_durs = inputs
        reconstructions, posterior = self(x)
        
        padding_mask = (ph == 475)
        pred_durs = self.pred_duration(ph, notedurs, pitches, notetypes, padding_mask)   # (B,T)
        dur_loss = F.mse_loss(
                    pred_durs.masked_select(~padding_mask),        # 只计算有效位置
        ...
        if optimizer_idx == 0:
            ...
            total_loss= aeloss + dur_loss
            
            self.log("aeloss", aeloss, prog_bar=True, logger=True, on_step=True, on_epoch=True)
            self.log("durloss", dur_loss, prog_bar=True, logger=True, on_step=True, on_epoch=True)
    ...
    def validation_step(self, batch, batch_idx):
        inputs = self.get_input(batch, self.image_key)
        inputs, f0, pitches, notedurs, notetypes, ph, ph_durs = inputs
        ...
        self.log("val/dur_loss", dur_loss)
        self.log_dict(log_dict_ae)
        self.log_dict(log_dict_disc)
        return {**log_dict_ae, **log_dict_disc, "val/dur_loss": dur_loss}
