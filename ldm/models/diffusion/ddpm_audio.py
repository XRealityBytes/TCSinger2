...
class LatentDiffusion_audio(DDPM):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if bs is not None:
            x = x[:bs]
            batch['name'] = batch['name'][:bs]
            if 'f_name' in batch:
                batch['f_name'] = batch['f_name'][:bs]
        x = x.to(self.device)
        encoder_posterior = self.encode_first_stage(x)
        ...
        if self.model.conditioning_key is not None:
            ...
            if 'prompt_embed' not in c['acoustic'].keys():
                cc = c['acoustic']['prompt'].to(self.device)
                # print(cc.shape)
                # assert False
                cc=self.encode_first_stage(cc)
                zc=self.get_first_stage_encoding(cc).detach()
                c['acoustic']['prompt_embed']=zc
                
            # print(f'ccccccshape:{c["acoustic"]["acoustic"].shape}')
            
            if bs is not None:
                # print('name', batch['name'])
                # print('acoustic.shape 0', c['acoustic']['acoustic'].shape)
                # assert False,f'c[infer]:{c["infer"]}'
                for k in c.keys():
                    if k=='acoustic':
                        for i in c[k].keys():
                            c[k][i] = c[k][i][:bs]
    ...
    def forward(self, x, c, *args, **kwargs):
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.cond_stage_trainable:           # THIS !!!!!!!!
                c = self.get_learned_conditioning(c) # c: string -> [B, T, Context_dim] This should be a dict
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        if isinstance(cond, dict):
            ...
            # cond 本身就应该有 c_concat 和 c_crossattn 两个 key
            cond = {
                'c_concat': cond['acoustic'],
                'c_crossattn': cond['caption'],
                'name': cond['name'],
                'infer': cond['infer']
            }
        else:
        ...
        x_recon,loss,f0_pred = self.model(x_noisy, t, **cond)

        if isinstance(x_recon, tuple) and not return_ids:
            return x_recon[0],loss,f0_pred
        else:
            return x_recon,loss,f0_pred
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        model_output,loss = self.apply_model(x_noisy, t, cond)
    ...
    def p_mean_variance(self, x, c, t, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
        ...
        t_in = t
        model_out,loss,_ = self.apply_model(x, t_in, c, return_ids=return_codebook_ids)
    ...
    def progressive_denoising(self, cond, shape, verbose=True, callback=None, quantize_denoised=False,
        ...
        if cond is not None:
            if isinstance(cond, dict):
                ...
                for k in cond.keys():
                    if k=='acoustic':
                        for i in cond[k].keys():
                            cond[k][i] = cond[k][i][:batch_size]
                    elif k=='infer':
                        cond[k] = cond[k]
                    else:
                        cond[k] = cond[k][:batch_size]
            else:
    ...
    def sample(self, cond, batch_size=16, return_intermediates=False, x_T=None,
        ...
        if shape is None:
            # if self.channels > 0:
            #     shape = (batch_size, self.channels, self.mel_dim, self.mel_length)  # mel_length 只用来确定目标形状
            # else:
    ...
    def sample_log(self, cond, batch_size, ddim, ddim_steps, **kwargs):
        ...
        if ddim:
            ...
            samples, intermediates = ddim_sampler.sample(
                ddim_steps, batch_size, shape, cond, verbose=False,
                quantize_x0=kwargs['quantize_denoised'] if 'quantize_denoised' in kwargs else False, **kwargs)
