...
class SpectrogramDataModuleFromConfig(DataModuleFromConfig):
    ...
    def __init__(self, batch_size, num_workers, spec_dir_path=None, main_spec_dir_path=None, other_spec_dir_path=None,
                  mel_num=None, spec_len=None, spec_crop_len=1248, drop=0, cond_drop=None, mode='pad',
                 require_caption=True, train=None, validation=None, test=None, predict=None, wrap=False,
                 other_condition=None, max_tokens=80000):
        specs_dataset_cfg = {
...
class AudioLogger(ImageLogger):
    ...
    def _log_rec_audio(self, specs, f0, tag, global_step, pl_module=None, save_rec_path=None):
        ...
        for i, spec in enumerate(specs):
            ...
            if spec.shape[0] != 80:
                continue
            wave = self.vocoder.vocode(spec,f0=f0[i].squeeze(0).cpu().numpy())
    ...
    def _log(self, pl_module, images, batch_idx, split):
        ...
        for k in images: # images is a dict,images[k]'s shape is (B,C,H,W)
            ...
            if self.for_specs:
                ...
                if k in ['inputs']:
                    self._log_rec_audio(images[k], images['f0_gt'],tag, pl_module.global_step, pl_module=pl_module)                
                if k not in ['conditioning', 'conditioning_rec', 'att_nopix', 'att_half', 'att_det','inputs']:
                    self._log_rec_audio(images[k], images['f0'],tag, pl_module.global_step, pl_module=pl_module)
            else:
