...
class CodeUpsampleHifiGanGenerator(HifiGanGenerator):
    def __init__(self, h, c_out=1):
        super().__init__(h, c_out)

        self.code_num = h['code_num']
        self.codebook_num = h['codebook_num']
        self.code_emb_dim = h['code_emb_dim']
        self.unit_upsample_rate = float(h['unit_upsample_rate'])

        self.code_embed = nn.Embedding(self.code_num * self.codebook_num + 5, self.code_emb_dim)
        self.padding = self.code_num * self.codebook_num
        self.conv_pre = weight_norm(
            Conv1d(self.code_emb_dim * self.codebook_num, h['upsample_initial_channel'], 7, 1, padding=3))

    def forward(self, x, f0=None):
        assert x.shape[1] == self.codebook_num
        # print('x.shape', x.shape)
        # x.shape [B, 3, T]
        # # upsample
        ...
        #     x = F.interpolate(x.float(), size=(tgt_size,), mode='nearest').int()    #  if not float() first: RuntimeError: "compute_indices_weights_nearest" not implemented for 'Long'

        offsets = self.code_num * torch.arange(self.codebook_num)
        offsets = offsets.unsqueeze(0).unsqueeze(-1).to(x.device)
        x = x + offsets
        x[x > self.padding] = self.padding
        x = self.code_embed(x)  # [B, 3, T, C]
        x = x.transpose(2, 3)   # [B, 3, C, T]
        x = x.flatten(start_dim=1, end_dim=2)   # [B, C, T]

        # upsample
        if self.unit_upsample_rate != 1.:
            orig_size = x.shape[2]
            tgt_size = int(orig_size * self.unit_upsample_rate)
            x = F.interpolate(x, size=(tgt_size,), mode='linear')

        x = super().forward(x, f0)
